...
var noPasswordInDatabaseMessage = "No such password in our database";
var noLoginInDatabaseMessage = "No such login in our database";
var emailInDatabaseMessage = "Such email already exists";

//Get random predication
$(document).ready(function () {

    function nextPredication() {
        var index = Math.floor(Math.random() * cookies.length);
        $("#predictionText").empty().append(cookies[index].cookie);
    }

    nextPredication();

    $("#predictionNext").click(function () {
        nextPredication()
    });
});
...
$(document).ready(function () {
    ...
    function validateLoginForm() {
        ...
        if (!hasError) {
            if (!isLoginInDatabase(login)) {
                addErrorMessage(noLoginInDatabaseMessage);
            }

            if (!isPasswordInDatabase(password)) {
                addErrorMessage(noPasswordInDatabaseMessage);
            }
        }
    }
    ...
    function validateRegistrationForm() {
        ...
        if (!isCorrectEmail(email)) {
            ...
            addErrorMessage(emailInDatabaseMessage);
        }
