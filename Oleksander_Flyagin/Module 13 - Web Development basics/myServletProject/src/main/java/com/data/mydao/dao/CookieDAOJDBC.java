...
public class CookieDAOJDBC implements CookieDAO {
    ...
    private static String sqlDellet(Cookie cookie) {
        return "DELETE FROM USERS WHERE ID = " + cookie.getId() + ";";
    }
    ...
    public void create(Cookie cookie) {
        ...
        cookie.setId(findForIdlPrediction(cookie.getId(), cookie.getPrediction()).getId());
    ...
    public void delete(Cookie cookie) {
        try {
            ...
            int affectedRows = statement.executeUpdate(sqlDellet(cookie));
            if (affectedRows == 0)
